...
- **多芯片**：一个 `.icg` 用 `chip 名字 { ... }` 声明多块芯片，各编译成独立程序（各自 128 行 / 4 KiB 预算与 loader）；顶层 `const` / `data` / `func` 是所有 chip 共享的公共区。芯片间用 `bus 名字 { 槽位 num ... }` 声明命名网络通道、每个 chip 用 `use 名字 on dev:conn` 指定自己的访问点（`Bus.槽位` 读写 → `ChannelN`，唯一写者校验，`run` 按 bus 自动接线）。CLI `build` 按芯片写文件，`--chip NAME` 选一个，`--json` 的 `chips[]` 列出全部；`run` 在内置 VM 里多芯片锁步运行。
