...
- 差分/随机：`TestDifferentialRandom` 随机生成 2000 个 `.icg`（含 `const`、函数、`if/else`、`for`、`switch`、`break/continue`、`label`/`goto`、栈、批量、槽位、动态 `read/write`、`ins/ext`），分别用优化与 `IC10C_NO_OPT` 编译（并覆盖 `--stable-ins`/`--jump-table`/`--fast` 与 `data` 表），在 VM 中运行并对比设备写入序列（优化的回归安全网）
