...
数值字面量可以带单位后缀，编译期换算成游戏基准单位：

```go
const TARGET = 20c        // 摄氏   -> 293.15 K
const FREEZE = 32f        // 华氏   -> 273.15 K
const MAXPRE = 20.1MPa    // 兆帕   -> 20100 kPa
const ATM    = 101.3kPa   // 千帕（压力默认单位）
const GEN    = 1.5kW      // 千瓦   -> 1500 W
const WAIT   = 500ms      // 毫秒   -> 0.5 s
const HALF   = 50%        // 百分比 -> 0.5
d0.On = d1.Temperature > TARGET
d1.On = d2.Pressure > MAXPRE
if d3.Ratio < HALF { d4.On = 1 }
```

- 温度后缀 `c`（摄氏）、`f`（华氏）、`k`（开尔文，可省略）大小写均可，基准单位**开尔文**。
- 压力后缀 `Pa`、`kPa`（基准）、`MPa`、`bar`、`psi`，基准单位**千帕**。
- 功率后缀 `W`（基准）、`kW`、`MW`，基准单位**瓦**。
- 时间后缀 `ms`、`s`（基准）、`min`、`h`，基准单位**秒**。
- 比例后缀 `%` / `pct`，`50%` = `0.5`。
- 角度后缀 `deg` / `rad` **不换算**，仅作可读性标记。
- 后缀只作用于十进制字面量（`0x1f` / `0b10` 不受影响）。
- `%` 也是取模运算符：`50%` 是百分号，`50%2` / `50 % 2` / `a%2` 是取模。
...
`.icg` 的函数只是**编译期抽象**，编译后不存在函数边界：会被内联，或对多次调用的叶子函数编译成 `jal` 子程序（由编译器按体积取短）。因此：

- **不支持递归**（编译器会报错）。
- 函数本身没有调用开销，可以放心拆小函数让代码更好读。
...
顶层只允许 `const` 与 `data`（v0.1 没有可变全局变量）。
...
变量是函数内的**局部变量**，由寄存器分配器映射到 `r0`~`r15`，必要时溢出到宿主栈（`get/put db`）。
你不需要手动管理寄存器。
...
- 真实脚本示例：[`ic10code/`](ic10code/)（作者自有的 `.icg` 端口，与原 `.ic` 做 VM 等价性验证；第三方脚本仅本地保留）
- 编辑器支持：[`editors/vscode/`](editors/vscode/)（语法/语义高亮 + 片段 + 诊断 + 上下文补全 + 大纲/折叠/引用/重命名/参数提示/快速修复 + 编译预览 + VM 运行）

最好的学习方式：打开 `ic10code/` 里一个真实脚本，对照它同目录的原始 `.ic`，再用 `./ic10c build` 看编译结果。
