...
`ic10go` compiles a Go-like language (`.icg`) to **Stationeers IC10** machine code. With a hard budget of **128 lines / 4 KiB / 90 chars per line**, large hand-written IC10 scripts are painful; `ic10go` brings modern syntax (`if` / `for` / `switch`, `for range`, `case lo..hi` intervals, `if`/`switch` init statements, labeled `break`/`continue`, fully inlined functions, device properties, device stacks and sorter/printer stack-instruction builders, batch IO, channels, enums) plus register allocation, constant folding, CSE, LICM, branch fusion and stack spilling to fit the budget. A **persistent-stack data segment** (`data` tables, `switch ... table`) moves large tables out of the 128-line program. Toolchain: `ic10c` (`build` / `run` with a built-in VM / `fmt` / `decompile` / `minify` / `disasm` / `stats` / `lsp`) and a VSCode extension.
