...
> **前三个是编辑器侧的限制**（游戏内代码编辑器在粘贴/保存时的校验），芯片**运行时
> 并不检查**脚本长度；因此修改编辑器上限的**模组**可以允许更大的脚本，运行时照常执行。
> 上表是原版默认值（128 / 4096 / 90），`ic10c` 只是按同值做编译期校验，可用
> `--max-lines` / `--max-bytes` / `--max-line` 或 `IC10C_MAX_LINES` / `IC10C_MAX_BYTES` /
> `IC10C_MAX_LINE`（VSCode `icg.maxLines` / `icg.maxBytes` / `icg.maxLine`）对齐模组后的
> 编辑器上限；它不改变生成的机器码，也不会截断或填充脚本。
>
> 上表末行「执行：每 128 行自动暂停 1 tick」是**执行批/时钟**节奏，与编辑器上限无关，
> 通常不随模组一起变化。
...
- 溢出用宿主栈固定高地址：默认 `get/put db`（每次加载 1 行）；`--spill stack` 时
  `poke` 写入、恢复 `sp` 后 `peek` 读取。
...
> `--rel-jump` 把跳转改为相对形式（`jr` / `br*`）以省字节，但**逐条比较**：
> 只有相对形式严格更短时才用（`br*`/`jr` 多一个 `r`，负偏移多一个 `-`，小程序的
> 绝对行号往往更短）。相对基准（相对当前行还是下一行）需真机验证，默认关闭。
> 近似比较在条件中会融合为 `bap/bna/bapz/bnaz`；`if cond { call L }` 会融合为
> 单条 `b<cond>al`。
