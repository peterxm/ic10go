...
| 冗余读消除 | 复用更早的设备/槽位/批量读；含跨基本块 CSE 与 `put`→`get` 存储转发。**精确栈失效**：常量 `put db N`/`poke N` 只失效槽 N，`yield`/`sleep` 保留栈读（设备读仍失效） |
...
| 尾块合并 | 相同终结符的块共享相同的指令后缀；regalloc 后再按**物理寄存器**合并一次。默认不合并「结构相同但物理寄存器不同」的尾段；`--merge-renamed-tails` 开启后按结构合并，但要求重命名的寄存器在后缀后死亡且 live-in 读同色（见 [`tail-merge.md`](tail-merge.md)） |
| 死存储消除 | 覆盖前无读取的芯片栈写（`put`/`poke`，常量地址）；跨基本块用栈槽活跃性分析。默认栈持久故退出时全部存活；`private-stack` 下放宽（`yield` 不再是屏障，退出不必存活） |
...
1. **扁平翻译**（`Decompile`）：替换 `alias`/`define`，把 `r0..r15` 映射为同名变量（先读后写才发 `var`，其余首次赋值用 `:=`），控制流翻译为 `label`/`goto`/`call`/`ret`，`HASH()/STR()` → `hash()/str()`，`$hex`/`%bin` → `0x`/`0b`，已知 prefab 哈希还原为 `hash("Name")`，不支持的指令告警并留注释。
   ...
   - **计算跳转**：`j rX`/`jr rX`/条件分支到寄存器可表达为 `jump(expr)`；但计算**调用**（`jal rX`、`b<cond>al … rX`）没有 `.icg` 形式（`jump` 会丢 `ra`），因此告警并注释。
2. **结构化**（`DecompileStructured`，`-s`）：
   ...
   - 循环：只结构化**自然循环**（`(header, end]` 的块不得有区间外前驱）；循环头是 `jal`/`-al` 目标（尾调用状态机）不结构化。回边到循环头时，闩锁块转为 `break`（条件取反），内部块转为 `continue`（无条件回边直接 `continue`）。
   - 若无法保证每个基本块恰好发射一次，或产物无法编译，自动回退到扁平形式（`ic10.Decompile` 封装了该策略）。
   - 结构化仍是**尽力而为**：无法结构化的循环保留为 `goto`，因此不会误编译。

`ic10code/` 里的真实脚本（作者自有 + 本地保留的第三方脚本，后者缺失时自动跳过）都会做**反编译 → 重编译 → 设备写入序列对比**（`TestIc10CodeRoundTrip`，扁平与结构化各一遍）与 **minify 等价性**（`TestMinifyIc10Code`；minify 在展开 define 会超行宽时自动保留 define）。
