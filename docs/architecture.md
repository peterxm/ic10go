...
| 公共子表达式消除 | 复用相同表达式（基于可用表达式的全局 CSE，跨基本块） |
...
本地 `ic10code/` 语料（第三方，未随仓库分发；缺失时测试自动跳过）里的真实脚本都会做**反编译 → 重编译 → 设备写入序列对比**（`TestIc10CodeRoundTrip`）与 **minify 等价性**（`TestMinifyIc10Code`）；结构化是尽力而为，失败时回退到扁平形式。
