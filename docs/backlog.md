...
本次又给 outline 增加了**单个函数**候选（整套外提不如只提一个时能选到）。其余 opt-in
开关**不适合默认开启**，原因如下，故保持手动：
...
### D3 放宽 outline ⬜
现在只支持**叶子、单层**（IC10 只有一个 `ra`）。多层外提需要跨 `jal` 保存/恢复返回
地址，涉及持久栈交互，风险高，暂缓；单层内联体积决策已由 D2 的多方案取短覆盖。
