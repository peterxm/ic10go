...
| **P3** | C3 | 编译期**循环/生成器**（生成 `data`、展开模式） | 去重 + 省行 | 中 | ⬜ |
...
**方案**：`import "path"`（或 `ic10c build --lib DIR`）；被导入文件只能声明 `const`/`func`/`data`，按名字空间并入；检测循环导入；顶层声明冲突报错。

**验收**：两个文件共享一个 `const`/`func`；循环导入报错；导入内容不改变产物语义。
...
**问题**：N 个设备、N 路 PID 这类重复模式只能手抄。

**方案**：复用 P1 的编译期执行，提供一个生成 `data` 表 / 展开重复块的语法（例如 `data T = gen(n, i => f(i))` 或编译期 `for`）。

### A4 `data` 表由编译期函数生成

与 C3 同源：`data` 元素已是编译期表达式，P1 完成后即可 `data T = [f(i) for i in 0..n]` 形式。
