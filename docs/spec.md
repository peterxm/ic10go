...
- 函数内 `var` / `:=` 为局部变量，由寄存器分配器映射到物理寄存器，必要时溢出到宿主栈（`get/put db`）。
...
- **多返回值**：`func f(a num, b num) (num, num) { return a, b }`，调用点用
  `x, y := f(...)`（或 `x, y = f(...)`）解构。多返回值函数**始终内联**（IC10 只有一个
  返回地址与结果约定），结果落到各自的寄存器；它不能当普通表达式用，也不能出现在
  `const` / `data` 里。
- 无函数重载、无闭包。
- `return` 可省略类型（推导）。
...
也可以用**范围推导**在编译期把整张表生成出来：

```go
func sq(i num) num { return i * i }
data Squares = [ sq(i) for i in 0..255 ]   // 编译期展开成 256 个值
```

推导的上下界必须是编译期常量整数，长度受 512 槽持久栈限制；元素表达式可以调用
纯函数（见 §9.1）。
...
`bus` 声明一组命名网络通道；每个 chip 用 `use` 声明**默认访问点**，访问时也可内联覆盖：
...
chip display {
    func main() { for { yield(); d0.Setting = Display.o2Pressure[d2][1] } }
}
```

- `use Bus on dev:conn`：本 chip 的默认访问点（`dev` 为端口 `db`/`d0..d5` 或设备别名，
  `conn` 为字面量）。`Bus.slot` 用它；每次访问也可 `Bus.slot[dev][conn]` 覆盖。
- 通道号 = **槽位下标**（`Channel{i}`）；**bus 最多 8 槽**，超了拆成多个 bus。
- `Bus.slot` 写 → `s <dev>:<conn> ChannelN`，读 → `l r <dev>:<conn> ChannelN`。
- 每个槽位至多一个写者；**被读却没有写者** → 报错。不做握手，由用户保证时序。
- 同一槽位的收发双方必须落在同一条电缆网络（否则读到 `NaN`），跨网络需桥接设备。
...
- 路径相对**导入者所在目录**解析；省略扩展名时补 `.icg`。库搜索目录由
  `Options.LibDirs` 提供（CLI `--lib DIR`，VSCode `icg.libDirs`），在导入者
  目录之后按顺序查找。
...
- 编辑器（`ic10c lsp` / VSCode 扩展）**会跟随导入**：相对于当前文件的目录解析，
  因此被导入文件里的函数/常量在编辑器里也能解析、补全与跳转。
...
- `ireg(ptr)` 读取 `ptr` 指向的寄存器，`setIreg(ptr, v)` 写入（对应 IC10 的 `rrN`）。`ptr` 为常量时直接寻址该物理寄存器（`rN`，零拷贝）。
...
- 逻辑类型名（`Temperature` `On` `Ratio` `Setting` ...）是设备成员名，由内建表校验（可关闭：`IC10C_NO_CHECK=1`）。拼错时给出 `did you mean "Temperature"?` 建议；内建表由游戏 `LogicType` 枚举生成，见 [`target-ic10.md` §5.8](target-ic10.md#58-枚举表同步生成器)。
...
- 寄存器不足时**溢出到宿主栈**：固定高地址槽默认用 `get/put db` 读写（每次加载 1 行，不需要暂存寄存器）；`--spill stack` 回退为 `poke` 写入、保存/恢复 `sp` 后 `peek` 读取（最高寄存器保留作暂存）。
- `ra` 与 `sp` 不参与通用分配。
- `reserveRegs(lo, hi)` 声明的物理寄存器不参与分配，供 `ireg`/`setIreg`（`rrN`）间接访问。
...
枚举成员无需改编译器；`IC10C_NO_CHECK=1` 可关闭这些校验。
