...
// Format parses and re-prints .icg source in canonical form, preserving comments.
func Format(name string, src []byte) (string, *diag.Bag, error) {
	file := source.NewFile(name, src)
	diags := &diag.Bag{}
	toks, comments := lexer.TokenizeWithComments(file, diags)
	tree := parser.Parse(file, toks, diags)
	if diags.HasErrors() {
		return "", diags, nil
	}
	return ast.FormatComments(tree, comments), diags, nil
}
