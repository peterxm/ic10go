...
> 单位后缀（编译期换算）：温度 `c`/`f`/`k` → 开尔文；压力 `Pa`/`kPa`/`MPa`/`bar`/`psi` → 千帕；功率 `W`/`kW`/`MW` → 瓦；时间 `ms`/`s`/`min`/`h` → 秒；比例 `50%`/`30pct` → `0.5`/`0.3`；角度 `deg`/`rad` 不换算。
...
加 `--structured` 会基于后支配树还原 `if`/`else`/`for`（无法还原的部分自动回退到 goto）：
