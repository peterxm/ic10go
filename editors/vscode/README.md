...
- `SorterInstruction.` / `PrinterInstruction.` / `TraderInstruction.` / `ConditionOperation.` / `LogicReagentMode.` / `SorterStack.` / `PrinterStack.` / `LogicType.` / `DisplayMode.` / `Sound.` / `Color.` / `PowerMode.` → 枚举成员
